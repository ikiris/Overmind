...
import {DirectiveSKPatrol} from '../../directives/defense/patrolSK';
...
interface SourcePatrolOverlordMemory extends CombatOverlordMemory {
	targetLairID?: string;
}

/**
 * SourcePatrolOverlord -- spawns offensive creeps to allow source keeper mining
 */
@profile
export class SourcePatrolOverlord extends CombatOverlord {

	static requiredRCL = 7;

	memory: SourcePatrolOverlordMemory;
	...
	constructor(directive: DirectiveSKPatrol, priority = OverlordPriority.patrolSK.sourceReaper) {
		super(directive, 'patrolSK', priority, SourcePatrolOverlord.requiredRCL);
		this.priority += this.outpostIndex * OverlordPriority.patrolSK.roomIncrement;
		this.reapers = this.combatZerg(Roles.melee);
		this.defenders = this.combatZerg(Roles.ranged);
		this.memory = Mem.wrap(this.directive.memory, 'patrolSK');
	...
	private computeTargetLair() {
		...
		if (!this.targetLair) {
			const lair = this.pos.findClosestByRange(this.room.keeperLairs)
			this.targetLair = lair ? lair : undefined
		}
	}
